#FINAL ASSIGNMENT PLANET GAME
#AUTHORS: Cameron Savage, Melissa Lee(301300968), Ryan Thompson
...
def txttolist():
	...
	for x in range(len(localList)):
		...
		for i in range(len(localList[x])):
				newstr = newstr + localList[x][i] + "	 "
...
def AMAZINGEXPLODEXD(currentList,posi):
	...
	if donefor >= len(currentList): #Problem, sets this to the list
		...
		for r in range(1,donefor):
			...
			#if amazing explosion happens in a planet before the Python Planet
			#and/or player, move them back one index so they 'move back' with
			#the other planets and keep the proper corresponding index
			if donefor < py_planet:
...
def coolaliens(currentList,posi,level): ##when player confronts aliens
	...
	if level > int(effected[0]): ## aliens are less smart
		...
		print("Aliens have civ level: ", alienciv)
		...
		planFuel = int(effected[1])
		takeaway = random.randint(1,(p_fuel//2))
		newplanFuel = int(planFuel - takeaway)
		if(newplanFuel < 0):
			newplanFuel = 0
		planetinfo = []
		planetinfo.extend([effected[0],str(newplanFuel),effected[2]])
...
def collectrocks(currentList,posi): ##non fruitful function that appends collected rocks to global Arock variable
	...
	newinfo.extend([efPlanet[0],efPlanet[1],str(int(oldrocks - (oldrocks//3)))])
	currentList[posi] = newinfo
	Arocks.append(int(oldrocks//3))
	#print(currentList)
	print("You collected ",int(oldrocks//3)," rocks.")
...
while play == "y":
	...
	alive = True
	p_pos = 0
	win_type = "" ##records win type to be displayed in results

	planet_lst = txttolist() ##gathers list from text doc
	length = len(planet_lst)
	print("Please enter the following information to set up game: ") ##now user sets values ===>
...
#roll_dice(lst, pos)
        ...
        #calls circular list
#verification(prompt, cond1, cond2, vartype)
        #verifies user input

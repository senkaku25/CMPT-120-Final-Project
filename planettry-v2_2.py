...
def coolaliens(currentList,posi,level): ##when player confronts aliens
        ...
        ##print ("effected: ", effected) ##effected planet is the position the player is on list
        #effected = str(effected)
...
##                        planetinfo = planetinfo+effected[n]
                ...
                # 	planFuel = int(effected[3])+(10*int(effected[2]))
                # else:
                planFuel = int(effected[1]) #why 2 and not 1?
...
##	newinfo = newinfo+str(oldrocks - int(oldrocks//3)) ##int rounds the rocks to an int
...
def finalresults(currentList, posi):
	...
	print(presentlist(currentList,posi,py_planet))

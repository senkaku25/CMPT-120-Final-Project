...
def circularmovement(lst, pos, advance):
	...
	while counter < advance:
		...
		if pos > (length -1):
			pos = 0
	print("POS INSIDE CIRCULAR MOVEMENT: ", pos)
	return pos
...
while play == "y":
	...
	if py_planet_cond == "y":
		...
		py_planet = 10 #player position will never be equal to the position of the non-existant Python Planet
	...
	while p_pos != py_planet and moves < p_moves and alive == True:
		...
		print(move_choice)
		if move_choice == "r":
			print (roll_dice(planet_lst, p_pos))
			p_pos = roll_dice(planet_lst, p_pos)
			print("Returned POS:", p_pos)
